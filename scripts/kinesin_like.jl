if isdefined(@__MODULE__, :LanguageServer)
    ...
    attach_kinesin!(lattice, bead_info, conf.lattice.N, conf.lattice.S, Int(5*13 + 5))
    ...
    @showprogress for i in 1:Nt
        ...
        iterate!(lattice, bead_info, conf, conf.iter_pars,0)
        #iterate!(lattice1, bead_info1, conf, conf.iter_pars,1)
        #iterate!(lattice2, bead_info2, conf, conf.iter_pars,2)
        #iterate!(lattice3, bead_info3, conf, conf.iter_pars,3)
...
display(fig)
save("plots/cluster_growth_energy1.png", fig)
...
limits!(-30,30, -5, 24*5)
frame = Observable(1)
position1 = @lift([positions[$frame][i][2] for i in 1:length(lattice.x)])
position2 = @lift([positions[$frame][i][3] for i in 1:length(lattice.x)])
clrs = @lift([BEADCOLORS[false, clusters[$frame, i,1]] for i in 1:length(lattice.x)])

fig = scatter(position1, position2, color = clrs,
    axis = (title = @lift("Frame = $(round($frame, digits = 1))"), aspect=AxisAspect(1)), marker = :circle)
